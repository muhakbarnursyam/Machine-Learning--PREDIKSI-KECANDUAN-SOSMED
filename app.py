...
# ==========================================================
    ...
    if menu == "Prediksi Dataset Upload":
...
# ==========================================================
# FUNGSI PREPROCESSING & TRAINING UTILITY
# ==========================================================
...
# ==========================================================
    ...
    # Simpan file model hasil training
...
# ==========================================================
    ...
    st.header("📁 Upload Dataset Baru & Analisis Performa")
    st.info(
        """
        Silakan upload dataset CSV baru.
        - Jika dataset menyertakan kolom **'Addiction_Level'** (aktual), metrik performa komparatif & grafik akurasi akan ditampilkan.
        - Jika tidak menyertakan kolom aktual, sistem akan langsung menampilkan tabel hasil prediksi massal dari semua model.
        """
    )
    ...
    # Ambil data kloning untuk kebutuhan pemrosesan data
    prediction_data = new_data.copy()

    # Cek keberadaan target pembanding (aktual) di dataset baru
    has_target = "Addiction_Level" in prediction_data.columns

    if has_target:
        actual_labels = prediction_data["Addiction_Level"].copy()
        prediction_data = prediction_data.drop(columns=["Addiction_Level"])
    else:
        actual_labels = None

    # Bersihkan kolom-kolom non-fitur jika terdapat pada berkas yang diupload
    drop_cols = ["Student_ID", "Addicted_Score", "Sleep_Addiction_Indicator", "Physical_Activity_Indicator"]
    drop_cols = [col for col in drop_cols if col in prediction_data.columns]
    prediction_data = prediction_data.drop(columns=drop_cols)

    # ------------------------------------------------------
    # VALIDASI KETAT DAN DIAGNOSIS NAMA KOLOM
    # ------------------------------------------------------
    missing_columns = [col for col in feature_columns if col not in prediction_data.columns]
    
    if missing_columns:
        st.error("❌ Gagal Memproses: Kolom pada dataset yang Anda upload tidak lengkap!")
        
        col_diag1, col_diag2 = st.columns(2)
        with col_diag1:
            st.markdown("**⚠️ Kolom yang Dibutuhkan oleh Model:**")
            st.write(feature_columns)
        with col_diag2:
            st.markdown("**📋 Kolom yang Ditemukan di File Anda (Setelah Drop-List):**")
            st.write(list(prediction_data.columns))
            
        st.info("💡 **Solusi:** Pastikan nama kolom di file CSV baru Anda memiliki ejaan huruf besar-kecil (*case-sensitive*) dan tanda spasi yang persis sama dengan dataset awal.")
        st.stop()

    # Pastikan urutan kolom sesuai dengan training awal
    prediction_data = prediction_data[feature_columns]

    # Jalankan encoding kategori menggunakan modul tersimpan
    for col, encoder in feature_encoders.items():
        try:
            prediction_data[col] = encoder.transform(prediction_data[col].astype(str))
        except Exception as e:
            st.error(f"❌ Nilai kategori baru pada kolom '{col}' tidak dikenali oleh model.")
            st.stop()

    # Normalisasi Fitur menggunakan Scaler yang disimpan
    prediction_scaled = scaler.transform(prediction_data)

    # ------------------------------------------------------
    # PROSES PREDIKSI DAN EVALUASI MULTI-MODEL
    # ------------------------------------------------------
    st.write("---")
    perbandingan_list = []
    result_data_all = new_data.copy()

    # Mengoperasikan data pada seluruh arsitektur model terdaftar
    for name, model in models.items():
        preds = model.predict(prediction_scaled)
        pred_labels = target_encoder.inverse_transform(preds)
        
        # Tempel kolom komparasi prediksi ke kerangka tabel
        result_data_all[f"Prediksi ({name})"] = pred_labels
        
        # Lakukan penilaian evaluasi jika ada target acuan
        if has_target:
            acc = accuracy_score(actual_labels, pred_labels)
            pre = precision_score(actual_labels, pred_labels, average="weighted", zero_division=0)
            rec = recall_score(actual_labels, pred_labels, average="weighted", zero_division=0)
    ...
    # Tampilkan output sesuai keberadaan label aktual
    if has_target:
        st.subheader("📈 Analisis Performa Model pada Dataset Baru")
        ...
        # Plot Diagram Komparasi Akurasi
        fig, ax = plt.subplots(figsize=(10, 4))
        bars = ax.bar(df_compare["Model"], df_compare["Accuracy"], color='#1E88E5')
        ax.set_ylabel("Akurasi")
        ax.set_title("Perbandingan Akurasi Model Baru")
        plt.xticks(rotation=15)
        ...
        st.subheader("📊 Hasil Prediksi Komparatif Seluruh Model")
        st.info("Catatan: Metrik evaluasi akurasi disembunyikan karena tidak ada kolom target 'Addiction_Level' sebagai nilai pembanding aktual.")

    # Tampilkan output data frame final
    st.subheader("📋 Data Lengkap Hasil Eksekusi Semua Model")
    st.dataframe(result_data_all, use_container_width=True)

    # Ekspor ke file CSV
    csv_bytes = result_data_all.to_csv(index=False).encode("utf-8")
    st.download_button(
        "⬇️ Download Hasil Prediksi Lengkap (CSV)",
        data=csv_bytes,
        file_name="hasil_prediksi_massal_multi_model.csv",
        mime="text/csv"
    )
